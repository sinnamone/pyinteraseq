...
parser.add_option('--outputcontrol', action="store", dest="outputcontrol", default=None,
                  help='BAM file derived from pyinteraseq_mapping.py'
...
parser.add_option('--domainstarget', action="store", dest="domainstarget", default=None,
                  help='Output tabular file derived from pyinteraseq_main_domain_definition.py'
                       ' with suffix _domain_definition.tab ',)
...
class EnrichmentProkaryotic(object):
    ...
    def bam2tabular(self, inputbam):
        """

        :return:
        """
        try:
            self.bedfile = pybedtools.BedTool(inputbam).bam_to_bed()
        except traceback:
            self.filelogerrorwrite(traceback.format_exc())
            self.filelogerrorwrite(msg89)
            sys.exit(1)
        else:
            self.filelogerrorwrite(msg90)
...
if __name__ == '__main__':
    ...
    print options.outputarget
    ClassEnrichment.bam2tabular(options.outputcontrol)
    ...
    #     fileinput=options.outputcontrol, idexit='_blastnbackgroundparsed')
    # DictEnrichment["blastntargetparsed"] = ClassEnrichment.blastninputparsing(
    #     fileinput=options.outputarget, idexit='_blastntargetparsed')
