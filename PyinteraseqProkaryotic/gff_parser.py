...
class InputParsing(object):
    ...
    def chrcount(self):
        ...
        try:
            ...
            self.chrdict = self.df1[0].to_dict()
            # output write
...
if __name__ == "__main__":
    ...
    DictInfo = dict()
    # print options.fastasequence
    # print options.gff
    # print options.outputfolder
    # print options.outputid
    ParserClass = InputParsing(optparseinstance=options)
    print ParserClass.fastasequence
    print ParserClass.gff
    print ParserClass.outputfolder
    print ParserClass.outputid
    DictInfo["fastatabular"] = ParserClass.ref2tabular()
    DictInfo["chrfasta"] = ParserClass.chrcount()
    DictInfo["fastareformat"] = ParserClass.tab2fasta()
    print DictInfo["chrfasta"]
    ...
    #     dfAnno['Chr'] = os.path.basename(args.gff.split('/')[-1]).split('.')[0]
    #     dfAnno.loc[dfAnno['Strand'] == '+', ['Start']] = dfAnno['Start'].astype(int).sub(1)
    #     dfAnno[['Chr', 'Start', 'End', 'Synonym', 'Product', 'Strand']].to_csv(
    #         args.outputfolder + args.outputid + '.gff', header=None, sep='\t', index=False)
    # except traceback:
    #     s
