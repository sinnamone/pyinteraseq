...
class BlastNlucleotide(Trimming):
    ...
    def conversionbam2sam(self, bamfile):
        ...
        try:
            subprocess.check_call(['/usr/bin/samtools', 'view', '-h', bamfile, '-o', ('/').join([self.out, ('').join([self.outputid, '.sam'])])], stderr=fnull, stdout=fnull)
        except traceback:
    ...
    def filtersam(self, samfile):
        ...
        try:
            with open( ("/").join([self.out, ('').join([self.outputid, '_filtered.sam'])]), 'w') as filtsam:
            	subprocess.check_call(['/usr/bin/awk','''BEGIN{FS=OFS="\t"}{if($3!=\"*\") print $0} ''', samfile], stderr=fnull, stdout=filtsam)
        except traceback:
            self.filelog.write(msg82)
            sys.exit(1)
        else:
	...
	try:
		...
		subprocess.check_call(['/usr/local/bin/bedtools','genomecov','-bg','-ibam', bamfile,'-g',self.genomesize], stderr=fnull, stdout=bedgraph)
        except traceback:
            self.filelog.write(msg82)
            sys.exit(1)
        else:
    ...
    def bedgraph2bw(self,bedgraph):
        ...
        fnull = open(os.devnull, 'w')
        try:
           subprocess.check_call(['/opt/bedGraphToBigWig', bedgraph, self.genomesize,self.out + ".bw"])
        except traceback:
            self.filelog.write(msg82)
            sys.exit(1)
        else:
    ...
    def indexkallisto(self):
        ...
        try:
            if os.path.exists(self.dbname):
                ...
                subprocess.check_call(['/opt/miniconda3/bin/kallisto', 'index', '-i', self.dbname, self.fastasequence], stderr=fnull, stdout=fnull)
    ...
    def mappingkallistotran(self, indexkallisto,avefragmlen, stdfragmlen, reads):
        ...
        fnull = open(os.devnull, 'w')
        try:
	   subprocess.check_call(['/opt/miniconda3/bin/kallisto', 'quant', '-i', indexkallisto,'--single','--pseudobam', '-l', str(avefragmlen), '-s', str(stdfragmlen),"-o","".join([self.outputfolder, ('').join(["".join([self.outputid,self.TrascriptTag]), ''])]),reads], stdout=fnull,stderr=fnull)
        except subprocess.CalledProcessError:
    ...
    def mappingkallistopaired(self, indexkallisto,gtf,genomesize,read1, read2):
        ...
        fnull = open(os.devnull, 'w')
        try:
           subprocess.check_call(['/opt/miniconda3/bin/kallisto', 'quant', '-i', indexkallisto, '-o', ('/').join([self.outputfolder, self.outputid]),'--genomebam', '--gtf', gtf, '--chromosomes', genomesize, read1, read2], stdout=fnull,stderr=fnull)
        except subprocess.CalledProcessError:
            self.filelog.write(msg60)
            sys.exit(1)
        else:
    ...
    def mappingkallisto(self, indexkallisto, gtf,genomesize,avefragmlen, stdfragmlen, reads):
        ...
        fnull = open(os.devnull, 'w')
        try:
            subprocess.check_call(['/opt/miniconda3/bin/kallisto', 'quant', '-i', indexkallisto, '-o', ('/').join([self.outputfolder, self.outputid]),'--genomebam', '--gtf', gtf, '--chromosomes', genomesize, '--single', '-l', str(avefragmlen), '-s', str(stdfragmlen), reads], stdout=fnull,stderr=fnull)
        except subprocess.CalledProcessError:
            self.filelog.write(msg60)
            sys.exit(1)
        else:
    ...
    def mappingkallistopairedttran(self, indexkallisto, read1, read2):
        ...
        fnull = open(os.devnull, 'w')
        try:
           subprocess.check_call(['/opt/miniconda3/bin/kallisto', 'quant', '-i', indexkallisto,'--pseudobam', '-o', ('/').join([self.outputfolder, "".join([self.outputid,self.TrascriptTag])]),read1, read2], stdout=fnull,stderr=fnull)
        except subprocess.CalledProcessError:
            self.filelog.write(msg60)
            sys.exit(1)
        else:
